...
# Script to generate fstab entries for ext4, NTFS, and btrfs filesystems based on blkid output
# Includes user confirmation, enhanced debugging, optional label application, and skips UUIDs already in /etc/fstab
...
# Extract UUIDs from /etc/fstab
declare -a existing_uuids
if [ -f /etc/fstab ]; then
    while IFS= read -r line; do
        if [[ $line =~ ^UUID=([^[:space:]]+) ]]; then
            existing_uuids+=("${BASH_REMATCH[1]}")
        fi
    done < <(grep -v '^#' /etc/fstab)
    echo "Existing UUIDs in /etc/fstab: ${existing_uuids[*]}" >> "$debug_log"
else
...
while IFS= read -r line; do
    ...
    # Check if the line contains TYPE="ext4", TYPE="ntfs", or TYPE="btrfs"
    if [[ $line == *TYPE=\"ext4\"* ]] || [[ $line == *TYPE=\"ntfs\"* ]] || [[ $line == *TYPE=\"btrfs\"* ]]; then
        echo "Matched ext4, ntfs, or btrfs: $line" >> "$debug_log"
        ...
        # Check if UUID is already in /etc/fstab
        skip_uuid=false
        for existing_uuid in "${existing_uuids[@]}"; do
            if [ "$uuid" = "$existing_uuid" ]; then
                echo "Skipping $device (UUID=$uuid) as it is already in /etc/fstab"
                echo "Skipping $device (UUID=$uuid) as it is already in /etc/fstab" >> "$debug_log"
                skip_uuid=true
                break
            fi
        done
        ...
        # Prompt to apply label to filesystem (ext4 or btrfs only, if different from current label)
        if { [ "$fs_type" == "ext4" ] && command -v e2label >/dev/null 2>&1; } || { [ "$fs_type" == "btrfs" ] && command -v btrfs >/dev/null 2>&1; }; then
            if [ "$mount_name" != "$label" ]; then
                ...
                if [[ "$apply_label" =~ ^[Yy]$ ]]; then
                    if [ "$fs_type" == "ext4" ]; then
                        ...
                        if btrfs filesystem label "$device" "$mount_name"; then
                            echo "Applied label '$mount_name' to $device"
                            echo "Applied label '$mount_name' to $device" >> "$debug_log"
                        else
                            echo "Warning: Failed to apply label '$mount_name' to $device"
        ...
        echo "Line did not match ext4, ntfs, or btrfs: $line" >> "$debug_log"
    fi
done < "$temp_blkid"
...
if [ ! -s "$temp_fstab" ]; then
    echo "No ext4, ntfs, or btrfs filesystems found or no new entries generated."
    echo "No ext4, ntfs, or btrfs filesystems found or no new entries generated." >> "$debug_log"
